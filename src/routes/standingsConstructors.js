...
function formattedStandings(rows) {
    ...
    rows.forEach((row) => {
        if (row.year != currentYear) {
            ...
            ConstructorStandings[ConstructorStandings.length - 1].ConstructorStandings.push(
                formattedConstructorStandings(row)
            );
        }
    });
...
router.get("", (req, res) => {
    ...
    conn.query(sql, (err, rows, fields) => {
        ...
        if (constructorStandings) json.MRData.StandingsTable.constructorStandings = constructorStandings;
