...
router.get("", (req, res) => {
    ...
    conn.query(sql, (err, rows, fields) => {
        ...
        if (circuit) json.MRData.StatusTable.circuitId = circuit;
        if (driver) json.MRData.StatusTable.driverId = driver;
        if (constructor) json.MRData.StatusTable.constructorId = constructor;
        if (grid) json.MRData.StatusTable.grid = grid;
        if (result) json.MRData.StatusTable.result = result;
        if (fastest) json.MRData.StatusTable.fastest = fastest;
        if (status) json.MRData.StatusTable.status = status;
        if (year) json.MRData.StatusTable.season = year;
        if (round) json.MRData.StatusTable.round = round;
