...
router.get("", (req, res) => {
    ...
    if ((driverStandings || constructorStandings) && (circuit || grid || result || status)) {
        res.status(400)
            .send("Bad Request: Cannot combine standings with circuit, grid, result or status qualifiers.")
            .end();
        return;
    }
    ...
    if (round) {
        ...
        if (driverStandings || constructorStandings) {
            if (year) sql += ` AND races.round=(SELECT MAX(round) FROM races WHERE races.year='${year}')`;
    ...
    conn.query(sql, (err, rows, fields) => {
        ...
        if (constructorStandings) json.MRData.ConstructorTable.constructorStandings = constructorStandings;
        if (driverStandings) json.MRData.ConstructorTable.driverStandings = driverStandings;
