...
router.get("", (req, res) => {
    ...
    sql +=
        " WHERE races.circuitId=circuits.circuitId AND qualifying.raceId=races.raceId AND qualifying.driverId=drivers.driverId AND qualifying.constructorId=constructors.constructorId";

    //Set the join
    if (grid || result || status || fastest)
        sql +=
            " AND results.raceId=qualifying.raceId AND results.driverId=qualifying.driverId AND results.constructorId=qualifying.constructorId ";
